alter table public.personal_courses rename to quizzes;
alter table public.personal_chapters rename to quizz_chapters;
alter table public.personal_notions rename to quizz_notions;

alter table public.quizz_chapters rename column course_id to quizz_id;
alter table public.quizz_notions rename column course_id to quizz_id;

alter table public.quizzes rename constraint personal_courses_id_owner_unique to quizzes_id_owner_unique;
alter table public.quizz_chapters rename constraint personal_chapters_id_course_owner_unique to quizz_chapters_id_quizz_owner_unique;
alter table public.quizz_chapters rename constraint personal_chapters_course_owner_fk to quizz_chapters_quizz_owner_fk;
alter table public.quizz_notions rename constraint personal_notions_course_owner_fk to quizz_notions_quizz_owner_fk;
alter table public.quizz_notions rename constraint personal_notions_chapter_course_owner_fk to quizz_notions_chapter_quizz_owner_fk;

alter index public.personal_chapters_owner_course_idx rename to quizz_chapters_owner_quizz_idx;
alter index public.personal_notions_owner_course_idx rename to quizz_notions_owner_quizz_idx;

alter policy personal_courses_own on public.quizzes rename to quizzes_own;
alter policy personal_chapters_own on public.quizz_chapters rename to quizz_chapters_own;
alter policy personal_notions_own on public.quizz_notions rename to quizz_notions_own;
  ...
  return exists (
    ...
    where c.id = (p_classification->>'courseId')::uuid and c.owner_id = v_owner
      and (nullif(p_classification->>'chapterId','') is null or exists (
        select 1 from public.quizz_chapters ch
        where ch.id = (p_classification->>'chapterId')::uuid
          and ch.quizz_id = c.id and ch.owner_id = v_owner
      ))
      and (nullif(p_classification->>'notionId','') is null or exists (
        select 1 from public.quizz_notions n
        where n.id = (p_classification->>'notionId')::uuid
          and n.quizz_id = c.id and n.owner_id = v_owner
  ...
  for v_entry in select value from jsonb_array_elements(p_payload->'questions') loop
    ...
    if v_entry#>>'{classification,kind}' = 'official' then
      ...
      if nullif(v_entry#>>'{classification,proposedChapterTitle}','') is not null then
        select id into v_chapter from public.quizz_chapters where owner_id=v_owner and quizz_id=v_course and title=v_entry#>>'{classification,proposedChapterTitle}' order by created_at limit 1;
        if v_chapter is null then insert into public.quizz_chapters(owner_id,quizz_id,title) values(v_owner,v_course,v_entry#>>'{classification,proposedChapterTitle}') returning id into v_chapter; end if;
      end if;
      if nullif(v_entry#>>'{classification,proposedNotionTitle}','') is not null then
        select id into v_notion from public.quizz_notions
        where owner_id = v_owner
          and quizz_id = v_course
        ...
        if v_notion is null then
          insert into public.quizz_notions(owner_id,quizz_id,chapter_id,title)
...
comment on table public.quizzes is 'Renamed from personal_courses; the courseId field persisted inside questions.classification is unchanged.';
comment on table public.quizz_chapters is 'Renamed from personal_chapters.';
comment on table public.quizz_notions is 'Renamed from personal_notions.';

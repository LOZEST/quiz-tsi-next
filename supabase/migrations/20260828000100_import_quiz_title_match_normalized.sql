-- ChatGPT question imports have no visibility into the user's existing
-- quizzes, so `proposedCourseTitle` rarely matches the exact title the user
-- typed when creating a quiz by hand, spawning a near-duplicate quiz on
-- every import instead of reusing the one the user already created. Match
-- case/whitespace-insensitively, and exclude soft-deleted quizzes so a
-- reused title can't silently resurrect content into an archived one.
...
begin
  ...
  for v_entry in select value from jsonb_array_elements(p_payload->'questions') loop
    ...
    if v_entry#>>'{classification,kind}' = 'official' then
      ...
      select id into v_course from public.quizzes where owner_id=v_owner and deleted_at is null and lower(trim(title))=lower(trim(v_entry#>>'{classification,proposedCourseTitle}')) order by created_at limit 1;
      if v_course is null then insert into public.quizzes(owner_id,title) values(v_owner,trim(v_entry#>>'{classification,proposedCourseTitle}')) returning id into v_course; end if;

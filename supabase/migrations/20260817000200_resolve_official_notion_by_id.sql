-- Fix: import_chatgpt_question_drafts required an exact match on BOTH
-- notion_id and chapter_id to resolve an official classification. Since
-- notion_id is the primary key of official_program_notions, any mismatch on
-- the client-supplied chapterId alone (e.g. the GPT reusing the document's
-- top-level chapter for every question instead of each notion's own
-- chapter) caused a valid notionId to be rejected as 'classification-unresolved'
-- even though it exists. Resolve by notion_id alone and treat chapterId as
-- an optional consistency signal that never blocks a known-good notionId.
...
begin
  ...
  for v_entry in select value from jsonb_array_elements(p_payload->'questions') loop
    ...
    if v_entry#>>'{classification,kind}' = 'official' then
      ...
      where notion_id = v_entry#>>'{classification,notionId}';

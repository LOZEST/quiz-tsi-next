...
La palette compacte de PR6 expose exactement quatre formes mathématiques prédéfinies : **Repère quadrillé**, **Repère gradué**, **Cercle trigonométrique** et **Tableau de signes/variations**. Chaque choix est une carte avec un aperçu graphique vectoriel fidèle à l'objet réellement placé. Le repère quadrillé possède un quadrillage régulier, deux axes centrés fléchés dans les deux sens et les labels `x` et `y`. Le repère gradué possède deux axes numériques, l'origine `O`, les graduations de `-6` à `6` en abscisse et de `-2,5` à `2,5` en ordonnée, ainsi qu'une grille discrète de points. Le cercle trigonométrique possède le cercle, les axes, `O`, `I`, `J` et l'indication courbe du sens direct. Le tableau reste un gabarit vide avec une première colonne étroite, une zone principale large et les trois libellés `x`, `signe de f'(x)` et `variations de f` ; aucune valeur critique, aucun signe ni aucune flèche arbitraire n'est prérempli. Droite, flèche, rectangle, carré, cercle simple, triangle, axes seuls et repère orthonormé historique ne sont pas exposés dans cette palette. Ils peuvent rester restaurables pour la compatibilité des anciennes scènes. La grille du Canvas reste un réglage secondaire. Les formes magiques, activées par défaut dans les réglages Pencil, transforment après 500 ms de maintien un tracé admissible en droite, cercle ou rectangle propre selon des seuils déterministes. Rectangle est classé avant cercle puis droite ; la fermeture, les quatre côtés, les coins et l'ajustement au périmètre empêchent qu'une boucle, une lettre ou une forme ouverte soit transformée. Le tracé et son snap constituent une seule action annulable.

En mode **Stylo**, **Effacer en griffonnant** est activé par défaut. Au relâchement seulement, un tracé dense présentant plusieurs inversions et croisements devient un effacement s'il traverse significativement au moins un trait ou une forme déjà présents. Les seuls objets réellement traversés sont supprimés, le griffonnage disparaît et l'ensemble constitue une transaction unique. Sans cible réellement touchée, si l'option est désactivée, ou après un snap magique, le tracé reste une écriture normale.

La gomme possède deux modes : **Objet**, par défaut, supprime intégralement chaque objet touché ; **Pixel** retire seulement la zone parcourue. Un mouvement rapide est interpolé sans trou. Les traits manuscrits sont découpés en fragments sérialisables conservant leur style ; les formes vectorielles utilisent un masque d'effacement versionné placé dans l'ordre de dessin, non sélectionnable, afin qu'un trait dessiné après le geste ne soit pas affecté. Un passage continu de gomme constitue une seule transaction Undo/Redo.

Sur le côté du tableau, deux boutons principaux seulement sont visibles : **Stylo** et **Formes**. Le premier est un bouton unique sans menu ni popup : à l'état initial il affiche l'icône, le libellé et le nom accessible **Stylo** ; une activation sélectionne immédiatement la gomme et remplace ces trois éléments par **Gomme** ; l'activation suivante restaure immédiatement **Stylo**. **Formes** révèle les quatre cartes mathématiques et un accès à la sélection des objets visuellement séparé, qui ne compte pas comme une cinquième forme. Aucun contrôle de taille prédéfinie n'est proposé ; les dimensions sont choisies naturellement lors du placement puis restent modifiables. Annuler et Rétablir restent disponibles dans un contrôle d'historique compact distinct.
...
Les réglages Pencil du menu ont un effet immédiat. Ils regroupent l'épaisseur du stylo, la grille, **Formes magiques**, **Effacer en griffonnant**, **Mode de gomme — Objet ou Pixel** et la main d'écriture. Aucun contrôle Rectangle magique, Griffonnage, mode Pixel ou taille de gomme n'apparaît sur le Canvas ou dans sa toolbar. Le menu n'expose pas logs, clés Supabase, diagnostics, toutes les statistiques, administration complète, gros bouton Déconnexion ni import/export permanent.

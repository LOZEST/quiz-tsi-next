...
Pour une classification `personal`, envoie toujours les six clés `kind`, `proposedCourseTitle`, `proposedChapterTitle`, `proposedNotionTitle`, `reason`, `requiresUserConfirmation` sur **chaque question**, même quand chapitre ou notion sont facultatifs : mets alors leur valeur à `null`, ne les omets jamais. `reason` doit être une vraie explication non vide propre à cette question (pas un texte générique recopié), et `requiresUserConfirmation` doit être littéralement `true`.

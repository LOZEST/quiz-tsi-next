...
6. Remplacer `PROJECT_REF` dans `openapi.yaml`, coller le schéma et `GPT_INSTRUCTIONS.md`, puis uploader le Knowledge généré.
7. Définir la Privacy Policy URL sur `/quiz-tsi-next/privacy/chatgpt-import`.
8. Tester les comptes A puis B, l’isolation, le replay `importId`, un payload hostile et un échec réseau.
9. Vérifier la couverture PDF, les incertitudes, l’absence de publication et la relecture locale après pull.

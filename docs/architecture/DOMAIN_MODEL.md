...
`QuizzListing` (`src/domain/quizz/QuizzListing.ts`) : `{ id, quizzId, ownerId, title, description, certified: boolean, hidden: boolean, averageRating: number | null, ratingCount, publishedAt, certifiedAt: string | null, hiddenAt: string | null }`. Un listing est visible dès sa publication (`hidden = false` par défaut) — il n'existe aucun statut `pending`/`approved`/`rejected`. `certified` et `hidden` sont deux booléens indépendants, pilotés uniquement par un admin : `certified` met en avant CE listing précis (pas l'auteur), `hidden` le retire de la marketplace sans toucher à sa certification.

`QuizzRating` (`src/domain/quizz/QuizzRating.ts`) : `{ id, listingId, raterId, score: 1..5, comment, createdAt }`. Seul un compte abonné (`quizz_listing_subscriptions`) peut noter un listing, contrôle appliqué côté serveur par le RPC `rate_quizz_listing`.

`QuizzMarketplaceGateway` (`src/domain/quizz/QuizzMarketplaceGateway.ts`) expose `publishQuizz`, `listVisibleListings`, `getListingPreview` (lecture seule), `subscribeToListing`/`hasSubscribed`, `rateListing`, `listSubscribedQuizzContent` et le triptyque admin `adminListListings`/`adminSetCertified`/`adminSetHidden`.

**Accès par référence.** S'abonner (`subscribeToListing`) n'effectue aucune copie de quizz/chapitre/notion/question : il insère une ligne dans `quizz_listing_subscriptions`, qui devient la condition d'une policy RLS accordant un accès en lecture au Quizz original (`questions`, `quizzes`, `quizz_chapters`, `quizz_notions`), en complément des règles déjà existantes `static`/possédé/`shared`. Le Quizz reste la propriété exclusive de son auteur — l'abonné ne peut ni l'éditer ni le republier comme le sien, et toute modification de l'auteur se répercute immédiatement côté abonné.
